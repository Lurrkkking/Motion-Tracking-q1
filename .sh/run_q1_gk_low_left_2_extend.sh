...
# Q1 goalkeeper low-left dive (extended reach) motion tracking. Reward: q1_gk_dive_left_* window (phase 0.10-0.35).
# 完整独立训练脚本（不再引用通用 sh，参数全部内联）。
#
# Usage:
#   bash run_q1_gk_low_left_2_extend.sh
#   NUM_ENVS=4096 CHECKPOINT=... bash run_q1_gk_low_left_2_extend.sh

MOTION_FILE="${MOTION_FILE:-humanoidverse/data/motions/q1/q1_gk_low_left_2_extend.pkl}"
EXP_NAME="${EXP_NAME:-Q1_GK_LowLeft2_Extend}"
NUM_ENVS="${NUM_ENVS:-2048}"
CHECKPOINT="${CHECKPOINT:-}"
DOMAIN_RAND="${DOMAIN_RAND:-domain_rand_base}"
REWARD_CONFIG="${REWARD_CONFIG:-motion_tracking/reward_q1_gk_low_left_2_extend}"
