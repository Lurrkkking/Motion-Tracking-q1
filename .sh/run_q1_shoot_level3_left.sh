...
# Q1 LEFT-foot shooting motion tracking (shoot_level3_left). Reward: q1_shoot_left_* window (phase 0.20-0.60).
# 完整独立训练脚本（不再引用通用 sh，参数全部内联）。
#
# Usage:
#   bash run_q1_shoot_level3_left.sh
#   NUM_ENVS=4096 CHECKPOINT=... bash run_q1_shoot_level3_left.sh

MOTION_FILE="${MOTION_FILE:-humanoidverse/data/motions/q1/shoot_level3_left.pkl}"
EXP_NAME="${EXP_NAME:-Q1_Shoot_Level3_Left}"
NUM_ENVS="${NUM_ENVS:-2048}"
CHECKPOINT="${CHECKPOINT:-}"
DOMAIN_RAND="${DOMAIN_RAND:-domain_rand_base}"
REWARD_CONFIG="${REWARD_CONFIG:-motion_tracking/reward_q1_shoot_level3_left}"
