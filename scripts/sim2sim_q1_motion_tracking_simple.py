...
def run_mujoco(cfg,
    ...
    # ---- 2. Load motion frame 0 for init (optional) ----
    motion_dof_pos_frame0 = None
    motion_root_pos_frame0 = None
    if motion_file and os.path.isfile(motion_file):
        try:
            import joblib
            mdata = joblib.load(motion_file)
            if isinstance(mdata, dict):
                mdata = mdata[list(mdata.keys())[0]]
            pose_aa = np.array(mdata["pose_aa"])
            root_trans = np.array(mdata["root_trans_offset"])
            # dof_pos = pose_aa.sum(axis=-1)[:, 1:]  # frame 0, joints 1..N
            dof_from_pkl = pose_aa[0, 1:, :].sum(axis=-1)
            # Some pkl files have extra passive bodies (e.g. head) — take first N joints
            motion_dof_pos_frame0 = dof_from_pkl[:cfg["num_actions"]]
            motion_root_pos_frame0 = root_trans[0].tolist()
            print(f"[MOTION] Loaded frame 0 from {motion_file}")
            print(f"[MOTION]   dof_pos @ frame 0 first6={np.array2string(motion_dof_pos_frame0[:6], precision=3)}")
            print(f"[MOTION]   root_pos @ frame 0 = {motion_root_pos_frame0}")
        except Exception as e:
            print(f"[MOTION] Failed to load motion file: {e}")
    ...
    for _ in range(sim_steps):
        ...
        # Render (strided)
        if renderer is not None and counter % render_every_steps == 0:
            if cam_follow_base:
    ...
    if renderer is not None and len(frames) > 0:
        actual_fps = len(frames) / max(elapsed, 1e-6)
        print(f"  Actual render FPS: {actual_fps:.1f}")

    # ====== Save video ======
    if renderer is not None and frames and video_out:
...
def parse_args():
    ...
    p.add_argument("--motion-file", type=str, default=None,
                   help="Path to reference motion .pkl (used only to extract frame 0 "
                        "dof_pos for initialization). No motion_lib needed.")
