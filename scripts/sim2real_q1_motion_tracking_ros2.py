...
def read_conf(config_file):
    ...
    cfg["cycle_time"] = float(config["cycle_time"])
...
def load_q1_joint_limits(dof_names):                                                                # 限位读取顺序 urdf-yaml-硬编码
    urdf_path = Path(_REPO_ROOT) / "humanoidverse/data/robots/q1/q1_22dof_box.urdf"
    try:
    ...
    q1_yaml = Path(_REPO_ROOT) / "humanoidverse/config/robot/q1/q1_22dof.yaml"       # yaml兜底
    try:
...
class Q1MotionTrackingNode(Node):
    ...
    def log_startup(self):
        ...
        self.get_logger().info(
            ...
            f"stand_fallback={'enabled' if self.stand_policy is not None else 'disabled'}"
        )
        self.get_logger().info(
    ...
    def update_get_ready_target(self):                                  # 插值，当前关节角 -> default_dof_pos
...
def parse_args():
    ...
    parser.add_argument("--config", type=str, default="scripts/q1_sim2sim_config.yaml")
    ...
    parser.add_argument("--stand-motion-file", type=str, default="")
...
def main():
    ...
    main_cycle_source = "config"
    if args.motion_file:
        main_cycle_time, main_cycle_source = infer_cycle_time_from_motion(args.motion_file)
        cfg["cycle_time"] = float(main_cycle_time)
    ...
    if args.stand_motion_file:
        ...
        # The node copies cfg into stand_cfg during construction; patching here lets
        # the stand policy use its own looping reference phase after switch-over.
    ...
    limits = load_q1_joint_limits(cfg["dof_names"])
    ...
    print(f"cycle_time={cfg['cycle_time']:.6f}s source={main_cycle_source}")
