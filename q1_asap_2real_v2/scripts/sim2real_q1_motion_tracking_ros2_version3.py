...
class Q1MotionTrackingNode(Node):
    def __init__(self, args, stand_cfg, mimic_cfg, limits, mimic_start_upper, mimic_start_source, stand_start_source):
        ...
        self.assert_joint_command_exclusive_if_needed()
    ...
    def assert_joint_command_exclusive_if_needed(self):
        if (
            self.args.mujoco_sim
            or self.args.dry_run
            or not self.args.enable_motors
            or self.args.allow_shared_joint_command_publishers
        ):
            return
        # Give DDS discovery a short moment so an existing high-level publisher
        # such as mc_ros2_node is visible before we start the 500 Hz command loop.
        deadline = time.monotonic() + 1.0
        foreign_publishers = []
        while time.monotonic() < deadline:
            publishers = self.get_publishers_info_by_topic(self.args.joint_command_topic)
            foreign_publishers = [
                info for info in publishers
            ...
            time.sleep(0.05)
        if foreign_publishers:
            names = sorted({f"{info.node_namespace.rstrip('/')}/{info.node_name}".replace("//", "/") for info in foreign_publishers})
            raise RuntimeError(
                f"Refusing --enable-motors because {self.args.joint_command_topic} already has "
                f"publisher(s): {names}. Stop high-level motion control / mc_ros2_node publisher "
                "or use low-level developer mode before running real motor output. "
                "If you are absolutely sure this is safe, pass --allow-shared-joint-command-publishers."
            )
    ...
    def state_age_ok(self):
        if self.args.mujoco_sim:
            # MuJoCo state is generated in-process by the command loop, not by ROS
            # topic callbacks. Viewer/physics startup can briefly block scheduling,
            # so the real-robot topic timeout would be a false positive here.
            return self.latest_joint_msg_time > 0.0 and self.latest_base_msg_time > 0.0
        now = time.monotonic()
...
def parse_args():
    ...
    parser.add_argument("--max-target-step-rad", type=float, default=0.2)
    ...
    parser.add_argument(
        "--allow-shared-joint-command-publishers",
        action="store_true",
        help="dangerous: allow real motor output even if another node already publishes /aima/hal/joint/command",
    )
